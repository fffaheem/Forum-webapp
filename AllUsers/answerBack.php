...
if(isset($_GET) and isset($_GET["sortBy"]) and isset($_GET["category"]) and isset($_GET["user"])){
    ...
    if($sortBy == "old"){
        if($category=="All"){
            $sql = "SELECT * FROM answers WHERE `username` = '$username' order by `answers`.`time` ASC";
        }else{
            $sql = "SELECT * FROM answers WHERE `username` = '$username' and `qCategory` = '$category' order by `answers`.`time` ASC";
        }
    }else if($sortBy == "New"){
        if($category=="All"){
            $sql = "SELECT * FROM answers WHERE `username` = '$username' order by `answers`.`time` DESC";
        }else{
            $sql = "SELECT * FROM answers WHERE `username` = '$username' and `qCategory` = '$category' order by `answers`.`time` DESC";
        }
        ...
        if($category=="All"){
            ...
            having answers.username = '$username'
            ...
            ON answers.a_sno = answer_like.a_sno
            GROUP BY answers.a_sno
            having answers.username = '$username' and answers.qCategory = '$category'
